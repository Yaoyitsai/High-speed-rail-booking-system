...
public:
   ...
   vector( const vector &right )
      : myData()
   {
       const size_type temps = right.myData.myLast - right.myData.myFirst;
       myData.myFirst = new value_type[temps]();

       for (size_type i = 0; i < temps; i++)
           myData.myFirst[i] = right.myData.myFirst[i];

       myData.myEnd = myData.myFirst + temps;
       myData.myLast = myData.myFirst + temps;
   }
   ...
   iterator erase( const_iterator where )
   {
       if (where >= myData.myFirst && where < myData.myLast)
       {
           ...
           for (unsigned int i = 0, j = pos; j < size(); i++, ++j)
               *(begin() + pos + i) = *(begin() + pos + i + 1);

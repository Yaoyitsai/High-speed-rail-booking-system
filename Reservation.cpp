...
void Reservation::displayReservationDetails()
{
    ...
    if (this->getAdultTickets() == 0 && this->getConcessionTickets() == 0)return;
    cout << std::right << setw(10) << "Date" << std::right << setw(10) << "Train No." << std::right << setw(8) << "From"
        ...
        << std::right << setw(8) << "Adult" << std::right << setw(12) << "Concession" << std::right << setw(8) << "Fare"
        << std::right << setw(11) << " Class\n";

    NorthboundTimetable ntrainTimetable;
    SouthboundTimetable strainTimetable;
    
    if (this->originStation > this->destinationStation) //Northbound
    {
        if (this->carClass == 2) {
            cout << std::right << setw(10) << this->date << std::right << setw(10) << this->trainNumber << std::right << setw(8) << station[this->originStation]
                << std::right << setw(10) << station[this->destinationStation]
                << std::right << setw(10) << ntrainTimetable.getDepartureTimes(this->trainNumber, 13 - this->originStation)
                << std::right << setw(9) << ntrainTimetable.getDepartureTimes(this->trainNumber, 13 - this->destinationStation)
                << std::right << setw(6) << adultTicketPrice[this->destinationStation][this->originStation] << "*" << this->getAdultTickets()
                << std::right << setw(10) << adultTicketPrice[this->destinationStation][this->originStation] / 2 << "*" << this->getConcessionTickets()
                << std::right << setw(8) << adultTicketPrice[this->destinationStation][this->originStation] * this->getAdultTickets() +
                (adultTicketPrice[this->destinationStation][this->originStation] / 2) * this->getConcessionTickets()
                << std::right << setw(10) << "Business";
        }
        else {
            cout << std::right << setw(10) << this->date << std::right << setw(10) << this->trainNumber << std::right << setw(8) << station[this->originStation]
                << std::right << setw(10) << station[this->destinationStation]
                << std::right << setw(10) << ntrainTimetable.getDepartureTimes(this->trainNumber, 13 - this->originStation)
                << std::right << setw(9) << ntrainTimetable.getDepartureTimes(this->trainNumber, 13 - this->destinationStation)
                << std::right << setw(6) << adultTicketPrice[this->originStation][this->destinationStation] << "*" << this->getAdultTickets()
                << std::right << setw(10) << adultTicketPrice[this->originStation][this->destinationStation] / 2 << "*" << this->getConcessionTickets()
                << std::right << setw(8) << adultTicketPrice[this->originStation][this->destinationStation] * this->getAdultTickets() +
                (adultTicketPrice[this->originStation][this->destinationStation] / 2) * this->getConcessionTickets()
                << std::right << setw(10) << "Standard";
        }
    }  
    else { // Southbound
        if (this->carClass == 2) {  
            cout << std::right << setw(10) << this->date << std::right << setw(10) << this->trainNumber << std::right << setw(8) << station[this->originStation]
                << std::right << setw(10) << station[this->destinationStation]
                << std::right << setw(10) << strainTimetable.getDepartureTimes(this->trainNumber, this->originStation)
                << std::right << setw(9) << strainTimetable.getDepartureTimes(this->trainNumber, this->destinationStation)
                << std::right << setw(6) << adultTicketPrice[this->originStation][this->destinationStation] << "*" << this->getAdultTickets()
                << std::right << setw(10) << adultTicketPrice[this->originStation][this->destinationStation] / 2 << "*" << this->getConcessionTickets()
                << std::right << setw(8) << adultTicketPrice[this->originStation][this->destinationStation] * this->getAdultTickets() +
                (adultTicketPrice[this->originStation][this->destinationStation] / 2) * this->getConcessionTickets()
